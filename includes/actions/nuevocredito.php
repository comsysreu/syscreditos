...
if($a=="crearcredito")
{
	...
	$montototal = $montocapital - $cuotadiaria;
		...
		if ($regnombre[0]['nombrecompleto']==$nombrecliente){	
			...
			if($creditos->insertRecord(array($idcliente,$idusuario,$cobrador,$fechainicio,$fechafinal,$idplan,$idmonto,$montointeres,$saldocapital,$saldointeres,$cuotadiaria,$estadoanular,$estadocredito,0))){	
				...
				for($i=1; $i<=$dias; $i++ ){
					...
					if ($detallecreditos->sqlTrans("INSERT INTO detallecreditos (idcredito, fecha, abonocapital, abonointeres, saldocapital, saldointeres, estado) VALUES ('$idultimo','$fechadias','$abonocapital','$abonointeres','$capital','$interes','$estadocuota')")){										
				...
				echo"<script>setTimeout(\"$('#divmodal2').modal('hide'); \",500); setTimeout(\"$('#divmodalNuevoCliente').modal('hide'); \",0); $('#divmodal').load('../includes/actions/nuevocredito.php?a=vistatotal&mt=$montototal&saldo=$saldoanterior&idcli=$idcliente').modal('show'); </script>";
				...
				<fieldset>
					...
					<div class='control-group'>
						...
						<div class='controls'>
							...
							echo"</select>
							<input type='text' id='dias' name='dias' style='visibility:hidden'>
	...
	echo"
		...
		<tbody>";
		    ...
		    foreach($registros as $filas => $campo) {
				...
				$fechafinal = strtotime (  '+ '.$diasfecha.' day' , strtotime ( $fecha ) ) ;
				$fechafinal = date ( 'd-m-Y' , $fechafinal );	

				
				echo"<td>";
					echo"<a rel='tooltip' title='Seleccionar' onclick=\" $('#nombreplan').val('".$campo['nombreplan']."'); $('#idplan').val('".$campo['idplan']."'); $('#fechafin').val('".$fechafinal."'); $('#dias').val('".$dias."'); $('#porcentajeinteres').val('".$campo['porcentajeinteres']."'); $('#divmodal3').modal('hide'); $('#divmodal2').modal('show'); \" ><i class='icon-check'></i></a>";
				echo"</td>";
				echo"</tr>";
